...
export default function HeaderActions() {
  const { quotations, totalCount, deleteQuotation } = useQuotations();
  ...
  function handleDelete(serialNo: number) {
    if (confirmDelete === serialNo) {
      deleteQuotation(serialNo);
      setConfirmDelete(null);
    } else {
      setConfirmDelete(serialNo);
      // auto-cancel confirm after 3s
      setTimeout(() => setConfirmDelete((cur) => cur === serialNo ? null : cur), 3000);
    }
  }
    ...
    <>
      ...
      <div className="pb-4 pt-2">
        <table className="w-full text-xs border-collapse">
          ...
          <tbody>
            {/* Saved quotation rows */}
            {quotations.map((q) => (
              <tr key={q.serialNo} className="hover:bg-white/5 transition-colors">
                ...
                <td className="border border-white/10 px-3 py-2 text-white font-semibold">
                  {q.partyName}
                </td>
                ...
                <td className="border border-white/10 px-2 py-2 text-center">
                  <div className="flex items-center justify-center gap-1.5">
                    ...
                    {/* Delete */}
                    <button
                      onClick={() => handleDelete(q.serialNo)}
                      className={`px-2 py-0.5 rounded text-[10px] font-bold transition-all ${
                        confirmDelete === q.serialNo
                          ? "bg-red-600 hover:bg-red-700 text-white animate-pulse"
                          : "bg-red-500/30 hover:bg-red-500/60 text-red-200"
                      }`}
                      title={confirmDelete === q.serialNo ? "Click again to confirm" : "Delete quotation"}
                    >
                      {confirmDelete === q.serialNo ? "Confirm?" : "🗑 Del"}
                    </button>
                  </div>
                </td>
              </tr>
            ...
            {/* Blank rows (always show at least 5 total) */}
      ...
      {/* ── Modals ── */}
      {showCreate && (
        <QuotationModal onClose={() => setShowCreate(false)} />
      )}
      {editQuotation && (
        <QuotationModal
