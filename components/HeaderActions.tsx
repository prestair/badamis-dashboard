    ...
    <>
      ...
      <div className="flex items-center justify-center gap-4 py-3 border-b border-white/10">
        ...
        <div className="flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-semibold bg-white/10 border border-white/30 backdrop-blur-sm">
          ...
          <span className="text-white font-bold text-base">{loading ? "…" : totalCount}</span>
          {totalCount > 0 && (
            <>
          ...
          <table className="w-full text-xs border-collapse">
            <thead>
              <tr className="bg-white/10 text-blue-100 text-[11px]">
                ...
                <th className="border border-white/15 px-3 py-2 text-center" style={{ width: 145 }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              ...
              {quotations.map((q) => (
                <tr key={q.dbId}
                  ...
                  {/* Actions cell — stop propagation so row click doesn't fire */}
              ...
              {Array.from({ length: blankRows }).map((_, i) => (
                <tr key={`blank-${i}`}>
                  <td className="border border-white/10 px-3 py-2.5 text-center text-white/30 font-semibold">{totalCount + i + 1}</td>
